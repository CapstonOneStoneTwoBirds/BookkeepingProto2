package com.capstone.bookkeepingproto2.OCR.abbyy.ocrsdk.android;
...
import com.capstone.bookkeepingproto2.OCR.com.abbyy.ocrsdk.Client;
import com.capstone.bookkeepingproto2.OCR.com.abbyy.ocrsdk.ProcessingSettings;
import com.capstone.bookkeepingproto2.OCR.com.abbyy.ocrsdk.Task;

import java.io.FileOutputStream;

public class AsyncProcessTask extends AsyncTask<String, String, Boolean> {

	public AsyncProcessTask(OCRResultsActivity activity) {
	...
	/** application context. */
	private final OCRResultsActivity activity;
	...
	protected Boolean doInBackground(String... args) {
		...
		try {
			...
			String language = "KoreanHangul,English"; // Comma-separated list: Japanese,English or German,French,Spanish etc.
			...
			*/
			...
			if( task.Status == Task.TaskStatus.Completed ) {
				...
				try {
					restClient.downloadResult(task, fos); // 오류 가능성 있음, 다시 확인
				} finally {
					fos.close();
				}
			...
			return true;
		} catch (Exception e) { // open failed: EROFS (Read-only file system) 발생
